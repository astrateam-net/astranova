...
# Function to run import-external-cluster.sh with OnePassword environment injected
function import_external_cluster() {
    log debug "Invoking import-external-cluster.sh with OnePassword environment variables"

    op run --env-file "${ROOT_DIR}/onepassword.env" -- "${SCRIPTS_DIR}/lib/import-external-cluster.sh"
}
